...
- 有效活跃度先过硬 gate：train/val 都必须至少 `5` 笔/月，持仓覆盖都必须至少 `5%`；低于这条线直接不合格，不允许成为 champion。目标仍是 `10-15` 笔/月，持仓覆盖率约 `16%` 起给满覆盖倍率。不要为了刷数量制造无收益短交易，也不要让大部分时间空仓。
...
- 默认策略画像是 `long / flat`。`short` 不是必须对称参与的主引擎，只在非震荡、下跌启动、弱反弹失败等高置信环境里作为辅助。short 占比和多空 capture 只做诊断，不进入评分；如果 short 不能改善整体 train/val 稳健收益、回撤或 val 弱块，可以主动收窄 short。
...
- 降权只因为 bear capture 弱就机械增加 short 的方案。扩大 short 必须说明它如何改善整体 train/val 稳健收益、回撤或 val 弱块。
