...
- 如果仍想留在“主簇过热”的方向簇里，`novelty_proof` 现在必须同时说明不同交易路径、至少两个预计会明显变化的关键诊断，以及为什么这不是只换 tag 或轻微阈值。
...
- 方向簇的 `ACTIVE_WINNER` 标签现在只保留给有实质正 `promotion_delta` 的方向；只有很小正增益但没真正抬过门槛的方向簇，不再被当成赢家簇提示给模型。
