...
当前评分已切到 Walk-Forward `eval + shadow` 双层结构：

- `selection_score` 只基于全部 `eval` 窗口表现
- `promotion_score` = `selection_score` 再叠加影子测试回归惩罚
- 优化模型只看到 `eval` 摘要，`shadow` 只在外层 gate / 晋级时使用
...
- **爆仓惩罚**：每次爆仓扣 1.5 分
- **影测落差惩罚**：`eval_avg - shadow_avg` 为正时，差值 x 0.15
- **负影测惩罚**：`shadow_avg < 0` 时继续扣分

门禁条件：

- 总交易 >= 30，`eval` 交易 >= 20，影测交易 >= 8
- 最大回撤 <= 65%
- 爆仓 <= 10 次
- `eval` 均值 > 0
- `eval` 正收益窗口占比 >= 35%
- 杠杆 >= 14x
- 影测均值 >= 0
- `eval_avg - shadow_avg <= 28`
- `shadow_avg` 不得比历史最优影测均值低超过 6 个百分点
...
- 评估范围：2025-04-01 ~ 2026-03-31
- 每块 20 天
- 除最后 2 块影子测试外，其余全部作为 Walk-Forward `eval`
- 影子测试结果不暴露给优化模型，仅用于外层拦截与晋级判断
