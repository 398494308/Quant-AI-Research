...
- 如果历史记忆里出现 `主簇过热（必须先读）`，默认必须切换到不同方向簇；只有在你能明确举证“将改变不同交易路径”时，才允许继续留在热簇。
...
- 如果出现 `主簇过热（必须先读）` 且你仍留在热簇，`novelty_proof` 必须同时明确说明：
  1. 这次改的是哪条不同交易路径
  2. 至少两个预计会明显变化的关键诊断
  3. 为什么这不是只换 tag、只换轻微阈值或只在同一 edited region family 里做小修补
- 如果最近多轮都零增益，优先切换方向簇或切换 edited region family，而不是继续围绕同一失败簇做近邻微调。
- 如果历史记忆里出现 `探索触发（必须执行）`，你必须把本轮当作探索轮，而不是继续做同簇微调。
- 探索轮优先切换方向簇；若无法跨簇，至少切换 edited region family、long-short target 或核心因子家族中的一项。
- 探索轮允许结果变差，但不允许只做措辞替换、标签换名、轻微阈值拨动或同一 edited region family 里的近邻改写。
