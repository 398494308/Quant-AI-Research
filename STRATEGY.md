...
研究目标不是追单月极值，而是优先提高验证集平均收益，并控制极端坏月和过拟合。

## 当前评分方式

研究循环的最终选优，已经不再直接使用单窗口回测结果中的内部 `result["score"]`。当前采用外层透明评分：
...
只有 `Gate 通过` 且 `overall` 高于当前最优，新的参数才会被正式保留。
...
5. 对 6 个时间窗口跑完整回测
6. 计算外层评分和 Gate
7. 更优则写回为新基底并发 Discord
8. 不够优则回滚代码，并把结果记入坏样本或普通历史记忆
