...
- 回测器已修正 `1h/4h` 上下文的取值时序，现在按高周期收盘完成时刻对齐 `15m` 决策点
- 回测器和分析脚本现在会先用全历史做指标 warmup，再截取评估窗口，避免窗口起点指标失真
- `freqtrade` 适配层已改成直接复用主策略入场函数，并尽量复用自研回测器的指标计算口径
- `scripts/freqtrade_compare.py` 现在对比的是“原始入场信号”的数量和具体时间戳集合，不再拿它去和“实际成交次数”混着比
- 修正以上口径后，旧的研究结果和旧的乐观回测收益都应视为过期，需要重新跑研究循环和回测

当前验证口径下：

- 默认对比窗口里，主策略原始入场时间戳集合和 `freqtrade` 适配层原始入场时间戳集合已经对齐
- 脚本会同时输出数量接近度、时间戳交集、Precision / Recall / F1
- 它现在更适合做“入场信号层是否接近”的检查
- 但仍然不能把它当成退出、分批止盈、加仓和权益曲线已经等价的证明
...
- `数量接近度` 只说明总信号数是否接近
- `时间戳F1` / `Jaccard` 才能说明两边是否在同一批 K 线上触发
- 如果看到 `100% / 100%`，也只代表原始入场层完全对齐
- 即便这些指标较高，也仍然不代表退出、`TP1`、`pyramid`、权益曲线已经对齐
